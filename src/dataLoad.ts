...
export function getTarget(target_input: any[], formatSettings: VisualSettingsModel): number {
  ...
  target = typeof target_input[0] === "string" ? target : ((target_input[0] || target_input[0]==0) ? target_input[0] : target) //if target is supplied as a measure then use that else use it from settings
  return target
}
...
export function fullData(host: IVisualHost, options: VisualUpdateOptions, formatSettings: VisualSettingsModel): SPCChartData {
  ...
  return {
    ...
    measureName,
    measureFormat: measureFormat_Return,
    decimalPlaces,
